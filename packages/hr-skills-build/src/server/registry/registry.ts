...
const RelevanceSignalSchema = v.pipe(
	v.object({ sourceSkill: v.string(), targetSkill: v.string(), coSelectionRate: v.number(), coSelectionCount: v.number(), observedCount: v.number() }),
	v.check(
		(signal) => signal.sourceSkill !== signal.targetSkill && signal.coSelectionRate >= 0 && signal.coSelectionRate <= 1 && Number.isInteger(signal.coSelectionCount) && signal.coSelectionCount >= 0 && Number.isInteger(signal.observedCount) && signal.observedCount > 0 && signal.coSelectionCount <= signal.observedCount && Math.abs(signal.coSelectionRate - signal.coSelectionCount / signal.observedCount) <= 1e-9,
		'Invalid relevance signal invariants',
	),
);

const RelevanceSignalTableSchema = v.pipe(
	v.object({ schemaVersion: v.literal(RELEVANCE_SIGNAL_SCHEMA_VERSION), generatedAt: v.string(), sourceDatasets: v.array(v.string()), totalObservations: v.number(), signals: v.array(RelevanceSignalSchema) }),
	v.check(
		(table) => Number.isInteger(table.totalObservations) && table.totalObservations >= 0 && table.signals.every((signal) => signal.observedCount <= table.totalObservations),
...
function hasValidSignalReferences(table: RelevanceSignalTable, allowedSkillIds: ReadonlySet<string>): boolean {
	...
	for (const signal of table.signals) {
		...
		if (seenPairs.has(pair) || !allowedSkillIds.has(signal.sourceSkill) || !allowedSkillIds.has(signal.targetSkill)) return false;
		seenPairs.add(pair);
	}
	return true;
}

export async function loadRelevanceSignalTable(path: string = RELEVANCE_SIGNALS_PATH, allowedSkillIds?: ReadonlySet<string>): Promise<RelevanceSignalTable | undefined> {
	let raw: string;
	try {
...
function extractDomainDependencies(domain: keyof typeof CATEGORY_META): string[] {
	...
	const deps: string[] = [];
	for (const match of meta.preamble.matchAll(SKILL_LINK_REGEX)) if (match[1]) deps.push(match[1]);
	return deps;
}
...
function rankRelatedSkills(skillId: string, tags: string[], sameDomainSkills: ReadonlyArray<{ id: string; tags: string[] }>, limit = 5): string[] {
	...
	return sameDomainSkills.filter((other) => other.id !== skillId).map((other) => ({ id: other.id, overlap: other.tags.filter((tag) => tagSet.has(tag)).length })).sort((a, b) => b.overlap - a.overlap || a.id.localeCompare(b.id)).slice(0, limit).map((entry) => entry.id);
}
...
export async function buildRegistry(signalTable?: RelevanceSignalTable): Promise<Registry> {
	...
	const draft = await Promise.all(skillIds.map(async (id) => {
	...
	const skills: RegistryEntry[] = draft.map((entry) => {
		const tier = computeTier(entry.paths.content, entry.paths.prompts, entry.paths.examples);
		const dependencies = entry.classification.category === 'uncategorized' ? [] : extractDomainDependencies(entry.classification.category as keyof typeof CATEGORY_META).filter((depId) => depId !== entry.id);
		...
		const relatedSkills = [...new Set([...(RELATED_SKILL_OVERRIDES[entry.id] ?? []), ...rankedRelated])].filter((id) => skillIdSet.has(id)).slice(0, 5);
		return { id: entry.id, name: entry.meta.name, version: entry.version, description: entry.meta.description, tier, domain: entry.classification.category, tags: entry.classification.tags, aliases: deriveAliases(entry.id), capabilities: entry.meta.supportedTasks, triggerPhrases: entry.meta.triggerPhrases, paths: entry.paths, dependencies, relatedSkills };
	}).sort((a, b) => a.id.localeCompare(b.id));

	return { schemaVersion: REGISTRY_SCHEMA_VERSION, generatedAt: new Date().toISOString().slice(0, 10), skillCount: skills.length, skills };
}
