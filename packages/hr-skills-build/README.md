...
Runs with [Bun](https://bun.sh). Runtime dependencies (`valibot`, `yaml`, and the workspace's own `hr-skills-ref`) are installed via `bun install` at the repo root — see `package.json` for versions. Builds with [tsdown](https://tsdown.dev). Interactive command-line workflows live in the separate [`hr-skills`](../hr-skills) workspace package. API documentation is generated by [`hr-skills-tsdoc`](../hr-skills-tsdoc) for the server and client surfaces.
