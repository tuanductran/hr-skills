...
Every HR skill lives in its own directory under `skills/`. A skill package consists of a required `SKILL.md` file and may include supporting knowledge in `content/` and practical workflows in `examples/`.
...
- `content/` and `examples/` are optional but recommended for larger skills.
