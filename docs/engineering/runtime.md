...
- **Real step execution** — implement a `StepExecutorFn` that loads a skill's `SKILL.md`, builds a prompt from `context`, calls a model, and returns its output. `packages/hr-skills/src/cli/execute-plan.ts` ships a stub executor as a CLI demonstration; production integrations should replace it.
