...
The source graphs are intentionally independent: a server module must not import from or re-export a client module, and a client module must not import from or re-export a server module. If both surfaces need the same behavior, maintain a surface-local implementation or type definition. This keeps future changes from accidentally pulling server code into a client bundle.

## Validation requirements

Every package with client/server surfaces must keep its entrypoints explicit in `package.json` and its build configuration. Client bundles should be scanned for `node:fs`, `node:path`, `node:child_process`, `server-only`, and process-derived repository path logic.
