...
> How decisions get made, who owns what, and how a contribution goes from an idea to a merged,
...
| `packages/hr-skills/`, `packages/hr-skills-build/`, `packages/hr-skills-ref/`, `packages/hr-skills-tsdoc/` | @tuanductran | Keeps the CLI, client/server libraries, TSDoc tooling, and validation/build workflows working; breaking changes here block every other PR via CI, so reviewed with extra care |
| `docs/` (specs: `format.md`, `registry.md`, `release.md`, `README.md`, this file) | @tuanductran | Source of truth for conventions; changes here should reflect settled decisions, not proposals — open an issue first for anything contentious |
